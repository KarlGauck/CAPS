...
fn calculate_orbit_basic_fixpoint(orbit: &Orbit, points_on_orbit: usize, iterations: usize) -> Vec<f64> {
...
pub fn ex1() {
    ...
    for orbit in orbits {
        let iterations_counts = vec!(100, 3, 2, 1, 0);

        let orbit_results: Vec<(Vec<(f64, f64)>, String)> = iterations_counts.iter().map(|iterations| {
            let eccentric_anomalies = calculate_orbit_basic_fixpoint(&orbit, 256, *iterations as usize);
            let points = calculate_orbit_positions(
                &orbit, eccentric_anomalies);
            (points, iterations.to_string())
        }).collect();
