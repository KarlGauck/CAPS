...
fn calculate_starting_mean_anomalies(points_on_orbit: usize) -> Vec<f64> {
    (0..points_on_orbit)
        .map(|x| ((x as f64) / (points_on_orbit as f64)) * 2. * f64::consts::PI)
        .collect()
}
            ...
            fixed_point_iteration(
                ...
                |old_eccentric_anomaly, _iterations| {
                    ...
                    let g = old_eccentric_anomaly
                        - orbit.eccentricity * old_eccentric_anomaly.sin()
                        - mean_anomaly;
...
fn calculate_orbit_positions(orbit: &Orbit, eccentric_anomalies: Vec<f64>) -> Vec<(f64, f64)> {
    let true_anomalies: Vec<f64> = eccentric_anomalies
        .iter()
        .map(|ecc_anomaly| {
            2.0 * f64::atan2(
                f64::sqrt(1. + orbit.eccentricity) * f64::sin(*ecc_anomaly / 2.0),
                f64::sqrt(1. - orbit.eccentricity) * f64::cos(*ecc_anomaly / 2.0),
            )
        })
        .collect();

    let sun_radii: Vec<f64> = true_anomalies
        .iter()
        .map(|true_anomaly| {
            orbit.semimajor_axis * (1.0 + orbit.eccentricity.powf(2.0))
                / (1.0 + orbit.eccentricity * f64::cos(*true_anomaly))
        })
        .collect();

    let points: Vec<(f64, f64)> = sun_radii
        .iter()
        .zip(true_anomalies)
        .map(|(sun_radius, true_anomaly)| {
            (
                sun_radius * f64::cos(true_anomaly),
                sun_radius * f64::sin(true_anomaly),
            )
        })
        .collect();
...
pub fn ex1() {
    ...
    for orbit in orbits {
        ...
        utils::plotting::line_graph(
            vec![
                (
                    default.iter().map(|(x, y)| (*x, y + y_offset)).collect(),
                    format!(
                        "Fixepoint:  {:?} Iterations",
                        default_result.1.iter().copied().reduce(usize::max).unwrap()
                    ),
                ),
                (
                    newton,
                    format!(
                        ...
                        newton_result.1.iter().copied().reduce(usize::max).unwrap()
                    ),
                ),
            ],
            PlotConfig::default()
                .title("Orbit (slight y-offset for better visibility)")
                .x_label("x in AU")
                .y_label("y in AU"),
    ...
    let planets = vec![
        PlanetPositionJ2000 {
            ...
            eccentricity: 0.0167,
            phi0: 102.95,
            mean_longitude: 100.46,
        }, // earth
        PlanetPositionJ2000 {
            semimajor_axis: 1.524,
            eccentricity: 0.0934,
            phi0: 336.04,
            mean_longitude: 355.45,
        }, // mars
    ];

    let orbits: Vec<Vec<(f64, f64)>> = planets
        .iter()
        .map(|planet| {
            let orbit = Orbit {
            ...
            let starting_mean_anomaly = planet.mean_longitude - planet.phi0
                + (j2000_diff as f64) * 2.0 * f64::consts::PI / orbital_period;

            let anomalies: Vec<f64> = (0..num_points)
                .map(|point| {
                    2.0 * f64::consts::PI * (point as f64 / points_per_year as f64) / orbital_period
                        + starting_mean_anomaly
                })
                .collect();

            let positions = calculate_orbit_positions(
                &orbit,
                calculate_orbit_newton_raphson(&orbit, &anomalies, precision).0,
            );
    ...
    let distances = orbits[0]
        .iter()
        .zip(&orbits[1])
        .map(|((x1, y1), (x2, y2))| f64::sqrt((x2 - x1).powf(2.0) + (y2 - y1).powf(2.0)))
        .collect::<Vec<_>>();
    let plot = (
        (0..num_points)
            .map(|x| (starting_date.year() as f64) + ((x as f64) / (points_per_year as f64)))
            .zip(distances)
            .collect::<Vec<_>>(),
        "Distance".to_string(),
    );
    ...
    utils::plotting::line_graph(
        vec![plot],
        PlotConfig::default()
            .title("Mars / Earth distance")
            .x_label("year")
            .y_label("distance in AU")
            .point_size(1),
...
pub fn ex3() {
    ...
    let f = |r: f64| g * big_m / r.powi(2) - g * small_m / (r_moon - r).powi(2) - omega.powi(2) * r;
    ...
    let precision = 1e-6;
    let (r_l1, iterations) =
        fixed_point_iteration(precision, r_moon / 2.0, |r, _| r - f(*r) / df(*r));

    println!(
        "L1 Lagrange point: {:.4e} m ({} iterations)",
        r_l1, iterations
    );
